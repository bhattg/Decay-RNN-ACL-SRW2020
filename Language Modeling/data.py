...
class SentenceCorpus(object):
    def __init__(self, lm_path, ccg_path=None, save_to='lm_data.bin', testflag=False,
                 ...
                 load_data=False, 
                 save_data=True,
                 save_train = "train_lm.bin", 
                 save_valid = "valid_lm.bin", 
                 only_save=False):
        if not testflag:
            if load_data:
                print(load_data)
                self.dictionary =  self.load_dict(save_to)
                self.load_train_data(os.path.join(lm_path, save_train))
                self.load_valid_data(os.path.join(lm_path, save_valid))
            else:
                self.dictionary = Dictionary()
                print(lm_path)
                ...
                if save_data:
                    print("Saving Train/Validation Data")
                    self.save_train_data(os.path.join(lm_path, save_train))
                    self.save_valid_data(os.path.join(lm_path, save_valid))
                    print("Data saved!!!")
            ...
            self.save_to = self.save_dict(save_to)   
            if only_save:
                print("ONLY SAVED USED! WE HAVE SAVED THE DATASET IN THE RESPECTIVE DIRECTORY.!! Aborted exection!!!!!!!!")
                sys.exit()
        else:
            self.dictionary = self.load_dict(save_to)
    ...
    def save_train_data(self, path):
        with open(path, 'wb') as f:
            torch.save(self.train_lm, f, pickle_module=dill)

    def save_valid_data(self, path):
        with open(path, 'wb') as f:
            torch.save(self.valid_lm, f, pickle_module=dill)

    def load_train_data(self, path):
        print(path)
        with open(path, 'rb') as f:
            self.train_lm = torch.load(f, pickle_module=dill)
    def load_valid_data(self, path):
        print(path)
        with open(path, 'rb') as f:
            self.valid_lm = torch.load(f, pickle_module=dill)
